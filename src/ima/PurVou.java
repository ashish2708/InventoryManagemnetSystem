...
public class PurVou {
	...
	private void initialize() {
		...
		JLabel lblNewLabel_1 = new JLabel("Purchase Voucher");
		...
		lblNewLabel_8.setBounds(437, 174, 128, 33);
		...
		int vno=0;
		String q10="select max(vno) from purchasevoucher";
		try {
				DatabaseConn c=new DatabaseConn();
				ResultSet rs=c.s.executeQuery(q10);
				rs.next();
				vno=rs.getInt("max(vno)");
				vno=vno+1;
		...
		try {
			DatabaseConn c=new DatabaseConn();
			String q1="Select firmname from supplier";
		...
		choice_2.setBounds(585, 174, 212, 25);
		try {
			DatabaseConn c=new DatabaseConn();
			...
			btnNewButton_6.setBounds(300, 182, 85, 21);
		...
		btnNewButton_1.setBounds(854, 170, 90, 29);
		...
		btnNewButton_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					String q="Select sid , address from supplier where firmname='"+s1+"'";
					ResultSet rs1=c.s.executeQuery(q);
					rs1.next();
					textField_1.setText(rs1.getString("sid"));
				...
				btnNewButton_5.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						for(int i=1;i<table.getRowCount();i++) {
							...
							String query="call purchase("+textField_8.getText()+","+textField_1.getText()+",'"+ProductId+"',"+ProductQty+",'"+textField_3.getText()+"')";
							try {
